...
struct ContentView: View {
    ...
    var body: some View {
        NavigationView {
            List{
                //5. Ahora deberemos chequear en el ForEach(courses) los cursos filtrados
                ForEach(courses.filter(shouldShowCourse)) { course in
                    ZStack{
    ...
    //1. Crearemos una funcion que nos permita realizar el filtrado de tipo booleano
    private func shouldShowCourse(course: Course) -> Bool {
        //2. Crearemos una constante para ver si se cumplen las condiciones(solo mostramos los comprados y nuestro curso esta comprado) ese curso sera mostrado o bien si no no esta marcada la opcion de de mostrar los cursos compados
        let checkPurchased = (settings.showPurchasedOnly && course.purchased) || !settings.showPurchasedOnly
        //3. La otra es que el precio de los cursos sea inferior al filtrado
        let checkPrice = (course.priceLevel <= settings.maxPrice)
        //4. Lo que devolveremos sera siempre y cuando se cumplan ambas condiciones (valor true)
