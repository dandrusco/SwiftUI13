...
struct ContentView: View {
    ...
    @State private var currentCourseURL = courses.first?.url
...
//                                    print("Me gusta")
                                    courses[self.lastCardIdx-1 % courses.count].likes.toggle()
                                    //isLikes!.toggle()
...
//                                    print("No me gusta")
            ...
            BottomBarView(url: currentCourseURL!)
    ...
    private func updateDeck(){
        let actual = courses[self.lastCardIdx % courses.count]
        currentCourseURL = actual.url
        deck.removeFirst()
        
        self.lastCardIdx += 1
        
        let newCourse = courses[self.lastCardIdx % courses.count]
        if newCourse.likes == false {
            let newCardView = CardView(course: newCourse)
            deck.append(newCardView)
        }else if newCourse.likes == true{
